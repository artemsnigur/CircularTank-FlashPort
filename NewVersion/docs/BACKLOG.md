...
| ~~**Enemy wall collision**~~ | ~~enemies do not turn at room bounds~~ — **shipped T112.** `PartGameArea.as:5370-5513`: every **non-boss** reflects off all four walls (reverse the perpendicular velocity, mirror the heading, only when it points into that wall); a **boss** never reflects and turns 1 deg/frame toward the tank instead (`:5516-5530`). The mechanism already existed and was faithful — **the gate was wrong**, applying it only in Defense where the AS3 splits on `enemyLevel != "B"` in every mode. Defense's bottom-edge crossing is unchanged (`:5449`). `turnTowardsGoal`'s `lockDirection` arm is ported but unreachable until the boss border AI (`:4642-4680`) lands. Detail in `HANDOFF.md` §5 | shipped |
| **Port enemy-enemy separation** — **CLOSED T123-T126 — all four passes shipped.** (a) `enemySeparation.ts`, pure + 27 tests, unwired. (b) `pushVel` threaded through `bounceOffWalls`/`clampToRoom` and onto `Enemy`, with decay wired in AS3 order — **no producer, so no behaviour change**: the 48 existing wall/steering tests pass with 0 deletions and `enemySteering.test.ts` untouched. (c) the pair loop in `GameplayScene`, in AS3 order (pairs for enemy *i* -> decay -> integrate), with `safetyDistance` rolled at spawn from `Math.random` — **not** `PM_PRNG`, which would desync background-prop layout. Driven A/B on one build via `?separation=0`, level 8-4, three repetitions: overlapping normal pairs **0.8/1.2/0.8% -> 0.3/0.7/0.2%** (40-79% fewer) and the worst penetration **0.065/0.348/0.229 -> 0.723/0.945/0.897** as a fraction of combined radii. Reachability proved separately: **0 effects with the loop off, 20-57 with it on**. (d) `BossCollision` wired to the boss-on-boss branch with the inlined gate at its **own** margin — `:5194` uses `distanceAdd = 200`, double every other site, and tests the *contact point* with no width term. Driven both ways on 3-9: contact inside the camera rect -> audible, queued 2x; contact 304 units outside -> audible false, queued 0x. **Correction from (b):** the `-y` branch differs from its three siblings in **three** ways, not one — its predicate omits the push, its bounds test omits the push, **and it never clears the push on contact** (`:5496` has no `pushVelY = 0`). Consequence with teeth: an enemy drifting down while shoved up harder than it drifts satisfies neither y predicate and does not move vertically **at all** that frame. **Scoping correction from (a):** the broad phase (`:2354`) is **direction-dependent** — it treats centres as corners and pads only in +x/+y, so when the subject is larger than the other body a real overlap can be invisible to one ordering. Swept over 241,816 overlapping configurations: **17.2% seen by exactly one ordering, 0% by neither**. A boss pair still always resolves because the surviving visit writes both bodies; a mismatched *normal* pair is nudged on one side only, and it is the smaller body that moves | `PartGameArea.as:5174-5221` — the pair loop that pushes two overlapping enemies apart by their relative mass (`:5199-5207` into `pushVelX/Y`, decayed at `:5365-5366`, integrated at `:5370-5385`, gated on `safetyDistance` at `:3354`/`:3358`). **The port has no enemy-enemy collision at all, so enemies interpenetrate on every one of the 405 levels.** **Adjacent to wall collision but not blocked by it** (T112): both live in the same integration expression, `xVel + pushVelX`, and with `pushVel` absent the wall rule reads `xVel` alone and is exact. When this lands, **do not make the four wall branches symmetric** — `:5488` gates on `yVel < 0` alone and omits `pushVelY` from its predicate while `:5493` still adds it. That asymmetry is the original's; the note is at the site in `enemySteering.ts`. That is the reason to build it: it is a visible fidelity gap with player value on its own. The `BossCollision` sound (`:5197`, boss-on-boss only) is a *byproduct* that falls out once the pair loop exists — **building the subsystem in order to unlock one sound would be backwards**, which is why this is filed as movement work and the sound is not listed separately any more | subsystem — touches enemy movement game-wide; **not in the active queue** |
