...
| **Sound: 15 silent** | **`--sound-sweep` still reports 47–48** and will keep doing so: it drives a *defeat*, and `Award1-3` fire on a **win**. They are confirmed firing by `--medals` instead, which is a different driven mode — so the sweep figure and the trigger-coverage figure have come apart, and neither is wrong. 10 of the 15 are wired and reach-only. The rest: `BossCollision` on enemy-to-enemy collision, `Achievement` on the toast queue, `Burning`/`FlameThrower` on an undriven loop channel, `ImpactCrazyCheese` a permanent orphan. Breakdown in `HANDOFF.md` §5 | mostly blocked, not owed |
