...
| ~~**M6**~~ | ~~Level Guide — 912 lines / 5 classes~~ — **shipped T102 (a-d); (e) recorded as having no counterpart.** The filing was wrong on four counts: **8** classes not 5, **951** lines not 912, **17** files reference it not 5, and the widget lives on the **shop**, not level select. Its `selectFromLevelGuide` half is unportable because this port's level select starts a level on click instead of selecting one | shipped |
