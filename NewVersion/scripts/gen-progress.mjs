...
lines.push('  Still untouched: the screen itself — routing, the display list, and the');
lines.push('  level-running loop it owns.');
lines.push('');
lines.push('  **Not wave spawning, which is `PartGameArea`\'s** — `ScreenGame` holds four');
lines.push('  spawn-related *statics* (`bossAmountSpawned`, `bossAmountSpawnedFull`,');
lines.push('  `multiplierSpawnRateHard/Medium`) and no spawning logic: 6 case-insensitive');
lines.push('  `spawn` hits against `PartGameArea`\'s 109. This block credited it with the');
lines.push('  subsystem next door, and read as a contradiction against `PartGameArea`\'s');
lines.push('  own list below, which records that subsystem as substantially ported.');
lines.push('');
lines.push('  The static count ran the same way round. Of its **131** `public static var`s,');
lines.push('  **107 are extracted** — the 40 `enemy*Stats`/`*BStats` tables above *plus*');
lines.push('  their 40 `*Strengths`/`*Weaknesses` companions, and 27 per-world');
lines.push('  `levelDataModelW*`/`enemyModelW*`/`flagModelW*` arrays. The **24**');
lines.push('  left are live run state (`hp`, `money`, `level`, `world`, `currentEnemies`,');
lines.push('  `enemiesLeft`, `flagsLeft`, the four `reloadTime*`, the weapon slots), not');
lines.push('  the "~90 remaining" this block used to claim.');
lines.push('');
...
lines.push('    - `weapons/firing.ts` — `tankAttack`\'s framework and **all 12');
lines.push('      primaries**: Cannon, MiniGun, Big Cannon, Gummy Bear Cannon,');
lines.push('      Shotgun, Penetration Cannon, Timed Bomb Cannon, Poison Cannon,');
lines.push('      Flamethrower, Cake Cannon, Laser Cannon and Magic Cannon —');
lines.push('      covering all eight damage channels and both damage paths. The');
lines.push('      mechanics once listed here as blockers are built and each has');
lines.push('      its own tests: the deterministic fan (`shotgun.test.ts`), beam');
lines.push('      (`laser.test.ts`), homing with a target budget');
lines.push('      (`magic.test.ts`), attach-and-detonate (`timedBomb.test.ts`),');
lines.push('      secondary projectiles (`cake.ts`), lifetime plus inherited');
lines.push('      velocity (`flames.ts`) and pass-through (`penetration.test.ts`).');
lines.push('    - `weapons/secondaries.ts` — the secondary firing block and');
lines.push('      `handleMines`, covering **1 of the 12 secondaries**: Mine, the');
lines.push('      free starter, and the only one that needs nothing unported.');
lines.push('      The other 11 are listed with their individual blockers in that');
lines.push('      file\'s header — thrown arcs with Ice/Poison explosion variants,');
lines.push('      homing rockets, persistent ground hazards, and the Shield,');
lines.push('      which is a timed tank state rather than a projectile.');
...
lines.push('  strength/weakness system. It is not portable as a single unit, but it');
lines.push('  has been taken in slices, and these are in:');
lines.push('    - `enemies/enemySteering.ts` — turn-toward-target, accelerate,');
lines.push('      clamp, integrate');
lines.push('    - `enemies/enemyFiring.ts` — the fire gate (`:6889`), the shot');
lines.push('      (`:6914`), bullet flight (`:1491`) and the hit on the tank');
lines.push('      (`:1574`). Two bullet classes (`Basic`, `BasicBoss`) across two');
lines.push('      patterns (`Front`, `Circle`)');
lines.push('    - `enemies/statusEffects.ts` — the three per-enemy timers that');
lines.push('      survive across frames: attached bomb (`:6312`), freeze (`:6326`)');
lines.push('      and poison damage-over-time (`:6366`), applied at `:5831`,');
lines.push('      `:5849` and `:5902`. **This is the shared timer the Timed Bomb');
lines.push('      and Poison Cannons were blocked on; it is built and wired** —');
lines.push('      `entities/Enemy.ts` carries a `StatusState` and ticks it');
lines.push('    - `enemies/enemyDeath.ts` — deaths that leave something behind');
lines.push('      (`:6825-6837`), currently the `Exploding` family');
lines.push('    - `enemies/damageTypes.ts` — the strength/weakness system');
lines.push('  Not ported: the **ten special mechanics**, each of which does have a');
lines.push('  real branch in the source — `Accelerating` (`:6697-6715`, read at');
lines.push('  `:5092`), `Temperamental` (`:6640-6667`, `:4610`, `:5094`),');
lines.push('  `Shrinking` (`:6772`), `GrapplingHook` (`:6789`), `DamageAddict`');
lines.push('  (`:4269` and `:4893-4917`), plus `Ghost`, `ScaredGhost`, `Medic`,');
lines.push('  `Teleporting` and `Trap`. Also outstanding: the other enemy bullet');
lines.push('  classes (`Following`, `Hook`, `Trap`) and three firing patterns,');
lines.push('  bullet reflection, the shield, the Ice/Poison explosion variants,');
lines.push('  and boss knockback fed back into the tank\'s velocity');
lines.push('  (`GameplayScene.ts:1224`).');
lines.push('  `enemies/enemyBehaviour.ts` is the live board for this — **9 of 20');
lines.push('  types implemented, 0 partial, 11 data-only** — and its figures are');
lines.push('  pinned by a test rather than restated here, so this paragraph cannot');
lines.push('  be the thing that drifts.');
lines.push('');
