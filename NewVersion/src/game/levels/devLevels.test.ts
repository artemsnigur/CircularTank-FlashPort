...
describe('the sentinel', () => {
  ...
  it('rejects real worlds and levels past the last theme', () => {
    ...
    /*
     * The end of the range, driven from both sides on adjacent numbers: the
     * last enemy's level is a dev level and the one after it is not. A range
     * that never ends passes the first half on its own.
     *
     * It moved to the theme levels in T248 and back here in T254 when the
     * gallery was retired — which is why it is computed from
     * `devEnemyTypes()` rather than written as a number.
     */
    const lastType = DEV_FIRST_SINGLE_LEVEL + devEnemyTypes().length - 1;
    expect(isDevLevel(DEV_WORLD, lastType), 'the last enemy level').toBe(true);
    expect(isDevLevel(DEV_WORLD, lastType + 1), 'one past it').toBe(false);
  });
});
